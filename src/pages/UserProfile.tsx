...
import { Pencil, Save, X, Upload, Trash2, User } from 'lucide-react';
  ...
  const handleMedicalRecordUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (!allowedTypes.includes(file.type)) {
        throw new Error('Invalid file type. Allowed types: PDF, JPG, PNG, DOC, DOCX');
      }
              ...
              <div className="mt-4">
                ...
                {medicalRecords.length >= 15 && (
                  <p className="text-red-600 text-sm mt-1">
                    Maximum limit of 15 records reached. Please delete some records to upload more.
                  </p>
                )}
              </div>
            </div>
